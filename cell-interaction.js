...
class CellInteraction {
    ...
    handleCellClick(e) {
        ...
        // Check if we're in player mode and have a selected attribute
        if (this.mapSystem.currentTab === 'builder' && this.mapSystem.playerMode !== 'viewer') {
            const selectedAttribute = this.mapSystem.selectedAttribute;
            
            if (selectedAttribute && selectedAttribute !== 'grassland') {
                // Check if this is a valid placement
                if (this.canPlaceBuilding(row, col, selectedAttribute)) {
                    // Send action to multiplayer system if available
                    if (window.multiplayerIntegration) {
                        this.sendBuildingAction(row, col, selectedAttribute);
                    } else {
                        // Single player mode - place directly
                        this.placeBuilding(row, col, selectedAttribute);
                    }
                }
            }
        }
    ...
    canPlaceBuilding(row, col, attribute) {
        // Basic validation - check if cell is not already occupied by a building
        const cell = this.mapSystem.cells[row][col];
        if (cell.attribute !== 'grassland' && cell.attribute !== 'water' && 
            cell.attribute !== 'mountain' && cell.attribute !== 'forest') {
            this.mapSystem.showNotification('Cannot place building here - cell is already occupied', 'warning');
            return false;
        }
        
        // Check multiplayer restrictions if in multiplayer
        if (window.multiplayerIntegration) {
            return window.multiplayerIntegration.canPlaceBuilding(attribute);
        }
    ...
    sendBuildingAction(row, col, attribute) {
        if (window.multiplayerIntegration) {
            // Send building placement action to server
            window.multiplayerIntegration.sendBuildingAction(row, col, attribute);
        }
    }
    
    placeBuilding(row, col, attribute) {
        // Place building directly (single player mode)
        const cell = this.mapSystem.cells[row][col];
        ...
        console.log(`Placed ${attribute} at (${row}, ${col})`);
    }
    ...
    getBuildingCosts(attribute) {
        const costs = {
            ...
            'residential': { wood: 60, ore: 20, commercialGoods: 15 },
